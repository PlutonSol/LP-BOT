...
Features:
1. Scans eligible reward markets and finds low-risk opportunities
   (resolution far away + high reward + low competition)
2. Telegram alerts when orders are close to being filled
3. Terminal dashboard of positions sorted by risk level

Usage:
    python bot.py scan          # Scan for best LP opportunities
    python bot.py run           # Start the full bot (scan + place orders + monitor)
    python bot.py dashboard     # Show terminal dashboard of current positions
    python bot.py setup         # Generate API credentials from your private key
"""
...
def fetch_reward_markets() -> list[RewardMarket]:
    ...
    # Fetch markets from Gamma API - filter for active, with rewards
    all_markets = []
    offset = 0
    limit = 100
    ...
    while True:
        try:
            events = gamma_get("/events", params={
                "active": "true",
                "closed": "false",
                "limit": str(limit),
                "offset": str(offset),
...
def parse_market(market: dict, event: dict) -> Optional[RewardMarket]:
    ...
    try:
        ...
        if rewards_daily == 0 and max_spread == 0:
        ...
        midpoint = (yes_price + no_price) / 2 if (yes_price + no_price) > 0 else 0.5
        # For binary markets the midpoint is essentially the yes price
        midpoint = yes_price
        ...
        # Competition score: based on liquidity depth relative to reward
        # Higher liquidity = more competition for the same reward
        if rewards_daily > 0:
            competition_score = min(100, (liquidity / (rewards_daily * 100)) * 10)
        else:
            competition_score = 100
        
        # Risk score: combines time to resolution, price extremity, and volume
        # - Markets closer to resolution are riskier (more volatile)
        # - Markets with extreme prices (near 0 or 1) are riskier
        # - Markets with high volume have more fills
        time_risk = max(0, 50 - days_to_resolution) * 2  # 0-100
        price_risk = (1 - abs(midpoint - 0.5) * 2) * 50  # Extreme prices = more risk
        volume_risk = min(50, volume_24h / 1000 * 10)     # High volume = more fills
        risk_score = min(100, (time_risk + price_risk + volume_risk) / 3)
        
        # Reward per dollar (daily)
        # Estimate: if we place min_size on both sides
